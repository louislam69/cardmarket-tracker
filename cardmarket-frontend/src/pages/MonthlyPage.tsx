...
export default function MonthlyPage() {
  ...
  return (
    <div style={{ padding: "1.5rem" }}>
      <h1 style={{ marginBottom: "4px" }}>Monatsvergleich (MoM)</h1>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
          ...
          <tbody>
            {items.map((item, i) => {
              ...
              return (
                <tr key={`${item.product_id}-${item.month}-${i}`} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  <td style={{ padding: "7px 12px" }}>{item.product_name}</td>
                  <td style={{ textAlign: "right", padding: "7px 12px" }}>{item.month}</td>
                  <td style={{ textAlign: "right", padding: "7px 12px", color: "#6b7280" }}>{item.prev_month}</td>
                  <td style={{ textAlign: "right", padding: "7px 12px" }}>{item.avg_realistic_price.toFixed(2)} €</td>
                  <td style={{ textAlign: "right", padding: "7px 12px" }}>{item.prev_avg_price.toFixed(2)} €</td>
                  <td style={{ textAlign: "right", padding: "7px 12px", color }}>
                    {(item.abs_change >= 0 ? "+" : "") + item.abs_change.toFixed(2)} €
                  </td>
                  <td style={{ textAlign: "right", padding: "7px 12px", color, fontWeight: 700 }}>
                    {(item.rel_change_pct >= 0 ? "+" : "") + item.rel_change_pct.toFixed(1)} %
                  </td>
                  <td style={{ textAlign: "right", padding: "7px 12px", color: "#6b7280" }}>{item.num_points}</td>
                </tr>
              );
            })}
          </tbody>
